...
awk '
{
    # Match lines starting with "!link:#element-<anystring>[AnyString]"
    if (match($0, /^!link:#element-([a-zA-Z0-9_-]+)\[[a-zA-Z0-9_-]+\](.*)$/, groups)) {
        # Replace with "!<<element-anystring>>" and keep the rest of the line
        print "!<<element-" groups[1] ">>" groups[2];
    } else {
